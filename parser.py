...
def load_cord(data, semantic_type, prefix, go_mapping, cc):
    res = {}
    for edge in data['edges']:
        node1 = edge['node1']['id']
        node2 = edge['node2']['id']
        if node1.startswith('GO:') and node1 in cc:
            node1_id = node1
        ...
        if node2.startswith('GO:') and node2 in cc:
            node2_id = node2
...
def load_data(data_folder):
    ...
    with open(go_mapping) as f2:
        ...
        cc = [k for k, v in go_mapping_data.items() if v == "BiologicalProcess"]

    res = load_cord(kg_data, "BiologicalProcess", 'go', go_mapping_data, cc)
